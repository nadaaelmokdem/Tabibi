...
{
    public class PatientService(IUnitOfWork unitOfWork, IFileService fileService) : Tabibi.Application.Interfaces.IPatientService
    {
        ...
        public async Task<ServiceResult> UpdateData(string userId, PatientExtraDTO patientData)
        {
            ...
            var user = await unitOfWork.PatientProfiles.Query().FirstOrDefaultAsync(p => p.UserId == userId);
        ...
        {
            var user = await unitOfWork.PatientProfiles.Query()
                .Include(p => p.User)
        ...
        public async Task<PatientDashboardDTO?> GetDashboard(string userId)
        {
            ...
            var chatSessionsCount = await unitOfWork.ChatSessions.Query()
                .CountAsync(cs => cs.PatientId == patient.PatientId && cs.Status == SessionStatus.Active);

            var unreviewedAppointments = await unitOfWork.Appointments.Query()
                .Include(a => a.Doctor).ThenInclude(d => d.User)
                .Include(a => a.Review)
                .Where(a => a.PatientId == patient.PatientId 
                         && a.Status == AppointmentStatus.Completed 
