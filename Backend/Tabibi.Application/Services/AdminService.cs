...
{
    public class AdminService(IUnitOfWork unitOfWork, UserManager<AppUser> userManager, IFileService fileService) : Tabibi.Application.Interfaces.IAdminService
    {
        public async Task<AdminDashboardDTO> GetDashboard()
        {
            ...
            var pendingDoctors = await unitOfWork.DoctorProfiles.Query()
                .Include(d => d.User)
            ...
            var totalAppointments = await unitOfWork.Appointments.Query().CountAsync();
        ...
        {
            return await unitOfWork.DoctorProfiles.Query()
                .Include(d => d.User)
                .Where(d => d.VerificationStatus == DoctorVerificationStatus.Pending)
                .Select(d => new PendingDoctorDTO
                {
        ...
        {
            var query = unitOfWork.DoctorProfiles.Query().Include(d => d.User).AsQueryable();
            ...
            var changeStats = await unitOfWork.DoctorProfileChangeLogs.Query()
        ...
        public async Task<List<DoctorProfileChangeLogDTO>> GetDoctorChanges(long doctorId)
        {
            ...
            var query = unitOfWork.DoctorProfileChangeLogs.Query()
        ...
        public async Task<List<AdminUserDTO>> GetAllUsers()
        {
            ...
            var roleByUser = await (
                ...
                join r in unitOfWork.Roles.Query() on ur.RoleId equals r.Id
