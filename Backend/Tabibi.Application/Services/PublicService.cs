...
{
    public class PublicService(IUnitOfWork unitOfWork) : Tabibi.Application.Interfaces.IPublicService
    {
        public async Task<List<SpecialtyDTO>> GetSpecialtiesAsync()
        {
            return await unitOfWork.Specialties.Query()
                .OrderBy(s => s.Name)
        ...
        {
            var query = unitOfWork.DoctorProfiles.Query()
                .Include(d => d.User)
